...
interface SwitchProps extends HTMLAttributes<HTMLDivElement> {
  ...
  /** Unchecked (off) track colour. Defaults to `var(--accent)`, which reads on
   *  the page background; override when the switch sits on a surface whose
   *  colour matches the accent, so the off state stays visible. */
...
const Switch = forwardRef<HTMLDivElement, SwitchProps>(
  ({ label, checked, onToggle, disabled = false, thumbTransition, offTrackColor = "var(--accent)", className, ...props }, ref) => {
    const labelId = useId();
        ...
        <SwitchPrimitive.Root
          ...
          style={{
            ...
            backgroundColor: checked
              ...
              : hovered
                // Darken the off track toward the foreground on hover — reads
                // as "more" in both themes (darker in light, lighter in dark),
                // never washing out to the surface behind it.
                ? `color-mix(in oklab, ${offTrackColor}, var(--foreground) 14%)`
                : offTrackColor,
          }}
